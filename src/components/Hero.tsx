...
export const Hero = () => {
  ...
  const slogans = [{
    text: "Dominate Amazon Search Results",
    icon: TrendingUp
  }, {
    text: "Beat Your Competition",
    icon: Award
  }, {
    text: "Reach Million+ Readers",
    icon: Users
  }, {
    text: "Get Reviews Dominance",
    icon: Star
  }, {
    text: "Create a Best Seller",
    icon: BookOpen
  }, {
    text: "Become a Success Story",
    icon: Sparkles
  }];
  useEffect(() => {
  ...
  return <section className="relative w-full min-h-screen flex flex-col overflow-hidden bg-slate-900">
        ...
        <div className="container mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-6">
              <Button variant="ghost" onClick={() => navigate('/help')} className="text-white/80 hover:text-white hover:bg-white/10 transition-colors">
                <HelpCircle className="w-4 h-4 mr-2" />
                Help Center
              </Button>
              <Button variant="ghost" onClick={() => navigate('/community')} className="text-white/80 hover:text-white hover:bg-white/10 transition-colors">
              ...
              {user ? <Button onClick={() => navigate('/dashboard')} className="bg-blue-600 hover:bg-blue-700 text-white">
                  Dashboard
                </Button> : <Button onClick={() => navigate('/auth')} className="bg-blue-600 hover:bg-blue-700 text-white">
                  Sign In
                </Button>}
            </div>
            ...
            <div className="lg:hidden flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={() => navigate('/help')} className="text-white/80 hover:text-white hover:bg-white/10 p-2">
                <HelpCircle className="w-5 h-5" />
              </Button>
              <Button variant="ghost" size="sm" onClick={() => navigate('/community')} className="text-white/80 hover:text-white hover:bg-white/10 p-2">
                <MessageSquare className="w-5 h-5" />
              </Button>
              {user ? <Button size="sm" onClick={() => navigate('/dashboard')} className="bg-blue-600 hover:bg-blue-700 text-white px-3">
                  Dashboard
                </Button> : <Button size="sm" onClick={() => navigate('/auth')} className="bg-blue-600 hover:bg-blue-700 text-white px-3">
                  Sign In
                </Button>}
            </div>
          </div>
      ...
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(50)].map((_, i) => <div key={i} className="absolute animate-float opacity-20" style={{
      ...
      <div className="absolute inset-0 pointer-events-none">
        {/* Floating Books */}
        {[...Array(8)].map((_, i) => <div key={`book-${i}`} className="absolute opacity-10 animate-float" style={{
        left: `${5 + i * 12}%`,
        top: `${15 + i % 3 * 25}%`,
        ...
        {/* Glowing Orbs */}
        {[...Array(6)].map((_, i) => <div key={`orb-${i}`} className="absolute rounded-full animate-pulse" style={{
        width: `${30 + i * 15}px`,
        height: `${30 + i * 15}px`,
        left: `${80 - i * 15}%`,
        top: `${10 + i * 15}%`,
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          <div className="w-full max-w-6xl mx-auto">
            ...
            <div className={`text-white transition-all duration-1000 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
              ...
              <div className="mb-4 sm:mb-6 h-16 sm:h-20 flex items-center">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  ...
                  <span key={currentSlogan} className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent animate-slide-in-right leading-tight">
              ...
              {/* Auth Section */}
              <div className="mb-8">
                {user ? <div className="flex items-center gap-4 bg-white/10 backdrop-blur-sm rounded-2xl px-6 py-4 border border-white/20">
                  ...
                  </div> : <Button variant="ghost" onClick={() => navigate('/auth')} className="bg-white/10 backdrop-blur-sm border border-white/20 text-white hover:bg-white/20 rounded-2xl px-6 py-3">
              ...
              <div className="flex flex-col gap-3 sm:gap-4 mb-6 sm:mb-8">
                <Button size="lg" className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white border-0 rounded-2xl px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg font-semibold shadow-lg hover:shadow-blue-500/25 hover:scale-105 transition-all duration-300" onClick={() => user ? navigate('/dashboard') : navigate('/auth')}>
                  <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 mr-2" />
                  {user ? 'Go to Dashboard' : 'Get Reviews Now'}
                  <ArrowRight className="ml-2 w-4 h-4 sm:w-5 sm:h-5" />
                </Button>
                
                <Button variant="outline" size="lg" className="w-full sm:w-auto bg-transparent border-2 border-white/30 text-white hover:bg-white/10 rounded-2xl px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg font-semibold hover:scale-105 transition-all duration-300" onClick={() => navigate('/community-signup')}>
              ...
              <div className="mt-6 sm:mt-8 p-4 sm:p-6 bg-black/40 backdrop-blur-sm rounded-2xl border border-white/20 shadow-lg">
                ...
                <Button variant="outline" size="sm" className="w-full sm:w-auto bg-white/10 border-white/30 text-white hover:bg-white/20 hover:border-white/50 transition-all duration-300" onClick={() => navigate('/affiliate')}>
            ...
            <div className={`relative mt-8 lg:mt-0 transition-all duration-1000 delay-300 ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              ...
              {[...Array(6)].map((_, i) => <div key={`float-${i}`} className="absolute animate-bounce" style={{
                left: `${10 + i * 15}%`,
                top: `${5 + i % 2 * 80}%`,
